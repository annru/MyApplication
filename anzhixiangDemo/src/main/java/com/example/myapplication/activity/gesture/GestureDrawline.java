...
//		paint.setColor(Color.rgb(240, 200, 30));// 设置默认连线颜色
        paint.setColor(context.getResources().getColor(R.color.red));// 设置默认连线颜色
...
//		paint.setColor(Color.rgb(240, 200, 30));// 设置默认连线颜色
        paint.setColor(mContext.getResources().getColor(R.color.red));// 设置默认连线颜色
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:// 当手指抬起的时候
                ...
                if (passWordSb.toString().length() == 0) {
                    break;
                }
                if (isVerify) {
                    // 手势密码校验
...
//		paint.setColor(mContext.getResources().getColor(R.color.color_FF7469));// 设置默认连线颜色
        for (Pair<GesturePoint, GesturePoint> pair : lineList) {
